...
前回は暗号技術の基礎として、機密性・完全性・認証・否認防止といった目的や、暗号技術の分類、安全性モデルについて見てきました。今回は**共通鍵暗号**に焦点を当てます。
...
[2.6 現実世界での応用例：理論が実践で活かされる瞬間](#26-現実世界での応用例理論が実践で活かされる瞬間)
[2.7 まとめ](#27-まとめ)
...
しかし、この技術には大きな課題があります。それが**鍵配布問題**です。

通信相手と秘密のメッセージをやり取りするには、まず同じ鍵を安全に共有する必要があります。しかし、インターネット経由で鍵をそのまま送信すると、盗聴者に鍵が漏れてしまいます。

現代ではこの問題を主に3つの方法で解決しています。公開鍵暗号による鍵交換（Diffie-Hellman鍵交換など）、物理的な鍵配布（安全な経路での事前配布）、PKI（公開鍵基盤）による信頼できる第三者機関の活用です。特にインターネット通信では、公開鍵暗号を使った鍵交換が標準的に使われています。
...
ストリーム暗号の歴史は、技術の進歩と攻撃手法の発見の歴史です。
...
この攻撃に対する対策として、**差分拡散性の高い設計**が重要です。S-box（換字ボックス）の最適化や、ラウンド関数での差分の拡散を確実に行う設計手法が採用されています。
...
辞書攻撃は、攻撃者が事前に準備した**辞書（リスト）** に含まれる候補値を順次試行する攻撃手法です。特にパスワードや鍵の生成に人間の選択が関与する場合に非常に有効です。辞書攻撃は、ブルートフォース攻撃よりも効率的で、実際の攻撃で頻繁に使用される手法です。
...
DESが設計された1970年代には、差分解読法や線形解読法は知られていませんでした。同様に、現在広く利用されているAESに対しても、将来新しい攻撃手法が発見される可能性があります。
...
ブロック暗号の安全性は、「**擬似乱数置換としてのふるまい**」や「**計算量的困難性の仮定**」に基づいています。ブロック暗号で重要な仮定は以下の通りです。
...
Feistel構造は、Horst Feistelが1970年代に考案した暗号設計手法です。**暗号化と復号が同じアルゴリズムで実現できる**のが特徴で、DESの他にも多くのブロック暗号で採用されています。
...
SPN構造（Substitution-Permutation Network）は、**置換（Substitution）と転置（Permutation）を交互に繰り返す**ことで高い安全性を実現する暗号設計手法です。Feistel構造とは異なるアプローチで、AESをはじめ多くの現代的なブロック暗号で採用されています。
...
暗号技術の安全性は、アルゴリズムの設計だけでなく、**鍵管理**の質によって大きく左右されます。どんなに優れた暗号アルゴリズムでも、鍵の管理が不適切であればセキュリティは保たれません。
...
暗号学的に安全な擬似乱数生成器（CSPRNG）を使用し、十分な不確実性（エントロピー）を持つ鍵を生成します。ハードウェア乱数生成器やOS提供の乱数機能を活用します。

#### 2. 鍵配布

暗号化通信とは別の安全な経路（帯域外配布）、Diffie-Hellman鍵交換などの鍵交換プロトコル、または公開鍵暗号基盤（PKI）を活用して配布します。

#### 3. 鍵使用

必要最小限の範囲でのみ鍵を使用し（最小権限原則）、同一鍵の過度な使用を避け、メモリ内での鍵を適切に保護します。

#### 4. 鍵更新

一定期間ごとに定期更新を行い、鍵漏洩の疑いがある場合は即座に緊急更新します。更新期間中は旧鍵のサポート（後方互換性）も考慮します。

#### 5. 鍵破棄

メモリやストレージから完全に削除し、機密データの痕跡を残さないよう上書きします。必要に応じてハードウェアを物理的に破壊します。
