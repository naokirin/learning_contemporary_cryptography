...
Webサイトへのアクセス、メールの送受信、オンラインショッピング、動画の視聴——これらすべての背後で、**共通鍵暗号**がデータを守っています。

前回、暗号技術の基礎として、機密性・完全性・認証・否認防止といった目的や、暗号技術の種類、安全性モデルに基づく現代暗号の安全性の概要について学びました。今回は、その中でも最も身近な技術の一つである**共通鍵暗号**を学びます。
...
共通鍵暗号の最大の魅力は、その**圧倒的な高速性**にあります。現代のコンピュータでAES-256（256ビット鍵）による暗号化を実行すると、1秒間に数GBのデータを処理できます。これは、公開鍵暗号の数百倍から数千倍の速度です。この速度の理由は、共通鍵暗号がビット操作を中心としており、CPUやハードウェアが直接サポートしている処理を中心にしており、ハードウェア最適化しやすいためです。
...
しかし、この技術にも大きな課題があります。それが**鍵配布問題**です。

友人と秘密のメッセージをやり取りしたいとします。共通鍵暗号を使うなら、まず同じ鍵を安全に共有する必要があります。しかし、インターネット経由で鍵をそのまま送信してしまうと、盗聴者に鍵が漏れる可能性があります。
...
この二つのアプローチは、異なる設計思想を持っています。ストリーム暗号は「流れるように、リアルタイムで」、ブロック暗号は「確実に、ブロック単位で」データを処理します。それぞれの特徴と使い分けを理解することで、現代の暗号システムがこれほど多様な用途に利用されている理由がわかります。

## ストリーム暗号

**ストリーム暗号**は、データを1ビットまたは1バイト単位で逐次暗号化していく方式です。最大の特徴は、データが到着するたび、即座に暗号化できることです。
...
**ブロック暗号**は、ストリーム暗号とは対照的に、固定長のブロック（通常128ビット）単位でデータを暗号化する方式です。データを一定のサイズに分割して、それぞれを確実に処理していきます。
...
1970年代に誕生したDESは、当時としては画期的な技術でした。しかし、コンピュータの計算能力の向上により、DESの安全性は次第に脅かされるようになりました。そして2001年、AESの標準化により、現代ブロック暗号は次第にAESへと移り変わっていきます。
...
これまで学んできた共通鍵暗号の理論は、私たちの身の回りの多くの技術で実際に活用されています。
このセクションでは、共通鍵暗号がどのように実世界で使われているかを、具体的な例を通じて見ていきます。
...
今回、共通鍵暗号について学びました。シンプルなアイデアから始まって、ストリーム暗号とブロック暗号の違い、DESからAESへの進化、暗号モードの重要性、そして実世界での応用まで——共通鍵暗号の全体を理解していきました。

現代のディジタルインフラにおいて、機密性がどのように担保されているかの一端を理解することができたと思います。
