...
```
平文: P1 P2 P3 P4 P5 P6 ...
鍵流: K1 K2 K3 K4 K5 K6 ...
暗号: C1 C2 C3 C4 C5 C6 ...
（Ci = Pi ⊕ Ki）
```
...
*暗号化アルゴリズム*
- **入力**: 平文ビット列 `m_1, m_2, ..., m_n`、秘密鍵 `k`
- **出力**: 暗号文ビット列 `c_1, c_2, ..., c_n`
- **動作**: 
  - 鍵 `k` から鍵ストリーム `k_1, k_2, ..., k_n` を生成
  - 各ビットを XOR: `c_i = m_i ⊕ k_i`

*復号アルゴリズム*
- **入力**: 暗号文ビット列 `c_1, c_2, ..., c_n`、秘密鍵 `k`
- **出力**: 平文ビット列 `m_1, m_2, ..., m_n`
- **動作**:
  - 鍵 `k` から同じ鍵ストリーム `k_1, k_2, ..., k_n` を生成
  - 各ビットを XOR: `m_i = c_i ⊕ k_i`
...
最も危険な攻撃手法の一つが鍵ストリームの再利用です。同じ鍵ストリーム `k_1, k_2, ..., k_n` で異なる平文 `m_1` と `m_2` を暗号化すると、暗号文は `c_1 = m_1 ⊕ k` と `c_2 = m_2 ⊕ k` となります。この時、攻撃者が両方の暗号文を入手できれば、`c_1 ⊕ c_2 = (m_1 ⊕ k) ⊕ (m_2 ⊕ k) = m_1 ⊕ m_2` となり、平文の差分が直接露出してしまいます。
...
攻撃者は、平文の差分 `ΔP = P ⊕ P'` と暗号文の差分 `ΔC = C ⊕ C'` の関係を利用し、差分特性と呼ばれる確率的な関係を構築します。この差分特性が高い確率で成立する場合、鍵の一部を効率的に推定できる可能性があります。
...
攻撃者は、平文の特定ビット `P[i_1] ⊕ P[i_2] ⊕ ... ⊕ P[i_k]` と暗号文の特定ビット `C[j_1] ⊕ C[j_2] ⊕ ... ⊕ C[j_l]` の間に線形関係が存在することを利用します。この線形近似式の成立確率が1/2から大きく偏っている場合、大量の平文・暗号文ペアを分析することで鍵の一部を推定できます。
...
Feistel構造の優れた点は、復号時も同じアルゴリズムを使用できることです。復号時は部分鍵を逆順（K_16, K_15, ..., K_1）で使用します：
...
*暗号化アルゴリズム*
- **入力**: 64ビット平文ブロック `P_64`、56ビット秘密鍵 `K_56`
- **出力**: 64ビット暗号文ブロック `C_64`
- **動作**:
  - 初期置換 (IP) を `P_64` に適用
  - 16ラウンドのFeistel構造処理（各ラウンドで48ビット部分鍵を使用）
  - 最終置換 (IP^-1) を適用して `C_64` を生成

*復号アルゴリズム*
- **入力**: 64ビット暗号文ブロック `C_64`、56ビット秘密鍵 `K_56`
- **出力**: 64ビット平文ブロック `P_64`
- **動作**:
  - 初期置換 (IP) を `C_64` に適用
  - 16ラウンドのFeistel構造処理（部分鍵を逆順で使用）
  - 最終置換 (IP^-1) を適用して `P_64` を復元
...
各バイト `s_ij` は8ビット（1バイト）を表し、全体で128ビットとなります。
...
- **数学的表現**: `c'_i = M × c_i` （Mは固定の4×4行列）
- **特徴**: 線形変換による拡散効果

**AddRoundKey（鍵加算）**
- **目的**: 鍵の影響を導入
- **処理**: 状態行列とラウンド鍵のXOR
- **数学的表現**: `s'_ij = s_ij ⊕ k_ij`
- **特徴**: 単純なXOR演算
...
*暗号化アルゴリズム*
- **入力**: 128ビット平文ブロック `P_128`、秘密鍵 `K` (128/192/256ビット)
- **出力**: 128ビット暗号文ブロック `C_128`
- **動作**:
  - 初期鍵加算: `state = P_128 ⊕ K_0`
  - N-1ラウンド: SubBytes → ShiftRows → MixColumns → AddRoundKey
  - 最終ラウンド: SubBytes → ShiftRows → AddRoundKey
  - 結果として `C_128` を出力

*復号アルゴリズム*
- **入力**: 128ビット暗号文ブロック `C_128`、秘密鍵 `K` (128/192/256ビット)
- **出力**: 128ビット平文ブロック `P_128`
- **動作**:
  - 逆初期鍵加算: `state = C_128 ⊕ K_N`
...
AESの鍵長は128ビット、192ビット、256ビットから選択可能であり、現在の技術ではブルートフォース攻撃は実現不可能です。AES-128の場合、鍵空間は `2^128 ≈ 3.4 × 10^38` 通りとなり、現在の最速スーパーコンピュータでも宇宙の年齢より長い時間が必要です。

AES-256の場合、鍵空間は `2^256 ≈ 1.2 × 10^77` 通りとなり、物理的に不可能な計算量となります。このため、**十分な鍵長の選択**により、ブルートフォース攻撃に対する安全性が確保されています。
...
| -------------------- | ------------------- | -------------------------------- |
| 56ビット（DES）      | 2^56 ≈ 7.2 × 10^16  | 数時間〜数日                     |
| 128ビット（AES-128） | 2^128 ≈ 3.4 × 10^38 | 宇宙の寿命より長い               |
| 256ビット（AES-256） | 2^256 ≈ 1.2 × 10^77 | 物理的に不可能                   |
