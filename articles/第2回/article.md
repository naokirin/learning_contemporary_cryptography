...
暗号技術の歴史を振り返ると、最も古くから使われてきたのは**共通鍵暗号**です。その基本概念は驚くほどシンプルです：**暗号化と復号に同じ鍵を使うこと**。この一見単純なアイデアが、実は現代のデジタル社会を支える最も重要な技術の一つなのです。
...
あなたが友人と秘密のメッセージをやり取りしたいとします。共通鍵暗号を使うなら、まず同じ鍵を安全に共有する必要があります。しかし、インターネット経由で鍵を送信してしまうと、盗聴者に鍵が漏れる可能性があります。
...
この二つのアプローチは、異なる設計思想を持っています。ストリーム暗号は「流れるように、リアルタイムで」、ブロック暗号は「確実に、ブロック単位で」データを処理します。それぞれの特徴と使い分けを理解することで、なぜ現代の暗号システムがこれほど多様な用途に利用され、強力なのかが見えてきます。
...
次回は「[**第3回　ハッシュ関数とメッセージ認証コード（MAC）**](https://qiita.com/naoki_hayashida/items/f477faba0d30debd909a)」について学びます。今回学んだ共通鍵暗号が「**機密性**」を主に担当するのに対し、次回の技術は「**完全性**」や「**認証**」を担当します。
