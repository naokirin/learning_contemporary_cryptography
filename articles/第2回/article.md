...
前回は暗号技術の基礎として、機密性・完全性・認証・否認防止といった目的や、計算量的困難性に基づく現代暗号の安全性について学びました。今回は、現代暗号の中核を成す1つである**共通鍵暗号**について詳しく見ていきます。
...
**共通鍵暗号（Symmetric-key encryption）** は、暗号化と復号に同じ鍵を使用する暗号方式です。この「同じ鍵」という特徴が、公開鍵暗号との大きな違いです。
...
- **Salsa20/ChaCha20**: 高速で安全なモダンなストリーム暗号。Google ChromeやGoogleのウェブサービスでのTLS/SSL通信、OpenSSHでの採用実績がある
