...
前回、私たちは暗号技術の基礎として、機密性・完全性・認証・否認防止といった目的や、計算量的困難性に基づく現代暗号の安全性について学びました。今回は、その中でも最も身近で、最も重要な技術の一つである**共通鍵暗号**を学んでいきます。
...
暗号技術の歴史を振り返ると、最も古くから使われてきたのは**共通鍵暗号**です。その基本概念は驚くほどシンプルです：**暗号化と復号に同じ鍵を使う**。この一見単純なアイデアが、実は現代のデジタル社会を支える最も重要な技術の一つなのです。

このシンプルな仕組みが、なぜこれほどまでに重要なのか？その理由を探ってみましょう。
...
想像してみてください。あなたが友人と秘密のメッセージをやり取りしたいとします。共通鍵暗号を使うなら、まず同じ鍵を安全に共有する必要があります。しかし、インターネット経由で鍵を送信してしまうと、盗聴者に鍵が漏れる可能性があります。これは「卵が先か鶏が先か」のような問題です。

この問題は、暗号技術の歴史において長い間、研究者たちを悩ませてきました。しかし、この挑戦こそが、後に公開鍵暗号の発明を促し、現代の暗号技術の基盤を築くことになったのです。
...
この問題の解決により、共通鍵暗号は真の実用性を獲得し、現代の通信システムの基盤となったのです。
...
この二つのアプローチは、まるで異なる哲学を持っています。ストリーム暗号は「流れるように、リアルタイムで」、ブロック暗号は「確実に、ブロック単位で」データを処理します。それぞれの特徴と使い分けを理解することで、なぜ現代の暗号システムがこれほど多様で強力なのかが見えてきます。
...
**RC4の登場**
RC4は1990年代から2000年代にかけて、SSL/TLSで広く使用されていたストリーム暗号でした。そのシンプルさと高速性から、多くのシステムで採用されました。しかし、2010年代に入って脆弱性が次々と発見され、現在では使用が推奨されていません。
...
次回は「**ハッシュ関数とメッセージ認証コード（MAC）**」について学習します。今回学んだ共通鍵暗号が「**機密性**」を担当するのに対し、次回の技術は「**完全性**」と「**認証**」を担当します。
