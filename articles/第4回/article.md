...
1. なぜ公開鍵で暗号化した暗号文が秘密鍵で復号できるのか？
2. なぜ秘密鍵なしでは復号できないのか？
3. なぜ素数を選ぶのか？
4. トーシェント関数 $\phi(n)$ とは何か？
5. 秘密指数 $d$ はどうやって求めるのか？
...
このセクション（4.3節）と次の4.4節では、より詳細な数学的背景を説明していきます。RSA暗号の数学的背景や詳細な高速化技術等に興味がない場合は、[4.5節](#45-rsa暗号の安全性向上)以降に進むこともできます。
...
ここで**位数**とは、群 $G$ の元 $g$ の位数は、$g^k = 1$ となる最小の正整数 $k$ です。
...
今回の記事では、公開鍵暗号のうち、RSA暗号の基礎と、その安全性を向上させるOAEP、署名への活用までを見てきました。さらに、単一秘密鍵のリスクから分散署名による解決策、そして暗号文のまま計算ができる準同型暗号という特別な技術の概要も学びました。
...
* [RSA8017 | PKCS #1: RSA Cryptography Specifications Version 2.2](https://datatracker.ietf.org/doc/html/rfc8017)
