...
2. $q$ を法として：$\gcd(m, q) = 1$ よりフェルマーの小定理が適用でき、$m^{q-1} \equiv 1 \pmod{q}$
   ...
   $$c^d \equiv m^{ed} \equiv m^{1 + k(p-1)(q-1)} \equiv m \cdot (m^{q-1})^{k(p-1)} \equiv m \cdot 1^{k(p-1)} \equiv m \pmod{q}$$
...
**Montgomery乗算**は、剰余乗算を高速化する革新的な技法です。
