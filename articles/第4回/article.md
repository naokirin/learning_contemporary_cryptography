...
**イデアル**：環 $R$ の部分集合 $I$ がイデアルであるとは、以下を満たすことです。
1. $I$ は加法について部分群
2. 任意の $r \in R$ と $i \in I$ に対して $r \cdot i \in I$

$\mathbb{Z}_n$ において、$d \mid n$（$d$ が $n$ を割り切る、すなわち $d$ は $n$ の約数である）なる $d$ に対して、$\{0, d, 2d, \ldots, n-d\}$ はイデアルです。

**例**：$\mathbb{Z}_6 = \{0, 1, 2, 3, 4, 5\}$ において、$d = 2$ のとき、$\{0, 2, 4\}$ はイデアルです。これは $2$ の倍数全体の集合で、加法について閉じており、任意の元との積もこの集合に含まれます。
...
**証明**：部分群 $H$ による左剰余類を考えます。任意の $g \in G$ に対して、剰余類 $gH = \{gh \mid h \in H\}$ を定義します。

1. 任意の2つの剰余類 $g_1H, g_2H$ は、$g_1H \cap g_2H \neq \emptyset$ ならば $g_1H = g_2H$
2. 各剰余類の要素数は $|H|$ に等しい
3. $G$ は互いに素な剰余類の和集合として表される

したがって、$|G| = k|H|$ なる正整数 $k$ が存在し、$|H|$ は $|G|$ の約数です。

**系**：有限群 $G$ の任意の元 $g$ の位数は、$G$ の位数の約数です。
...
**Montgomery乗算**は、剰余乗算を高速化する技法です。

**基本原理**：$a \cdot b \bmod n$ の計算を、$a \cdot b \cdot R^{-1} \bmod n$ の形に変換します。ここで $R$ は $n$ より大きく、$R$ と $n$ が互いに素な数です。

**Montgomery表現**：数 $x$ のMontgomery表現を $\tilde{x} = x \cdot R \bmod n$ と定義します。

**Montgomery乗算**：
$$\text{MontMul}(\tilde{a}, \tilde{b}) = \tilde{a} \cdot \tilde{b} \cdot R^{-1} \bmod n = \widetilde{a \cdot b}$$

この表現により、除算を避けて剰余計算を高速化できます。

### Sliding-Window法によるべき乗算の最適化

**Sliding-Window法**は、連続する0ビットを効率的にスキップしてべき乗算を高速化する技法です。

**アルゴリズム**：
1. ウィンドウサイズ $w$ を設定
2. 事前計算テーブル $\{a, a^3, a^5, \ldots, a^{2^w-1}\}$ を構築
3. 指数のビット列を $w$ ビットずつ処理
4. 連続する0ビットをスキップして効率化

**計算量**：通常のバイナリ法より約25%高速で、$O(\log k)$ 回の乗算で実現されます。
...
**Miller-Rabin素数判定法**：
- フェルマーの小定理の逆をより厳密に検証
- 奇素数 $p$ に対して、$p-1 = 2^s \cdot d$（$d$ は奇数）と分解
- 任意の $a$ に対して、$a^d \equiv 1 \pmod{p}$ または $a^{2^r \cdot d} \equiv -1 \pmod{p}$ が成り立つ
- 誤判定確率は $4^{-k}$ 以下（$k$ は試行回数）
...
**定理**：RSA暗号の暗号化・復号化は、$\log n$ の多項式時間で実行可能です。

**証明**：
1. **鍵生成**：$O((\log n)^4)$（確率的素数判定）
2. **暗号化**：$O((\log n)^3)$（バイナリ法）、$O((\log n)^2)$（Montgomery法）
3. **復号化**：$O((\log n)^3)$（通常）、$O((\log n)^3/4)$（CRT復号化）

すべての演算が $\log n$ の多項式時間で実行されるため、RSA暗号は効率的に実装可能です。
...
**2. 定数時間実装**：
- タイミング攻撃を防ぐため、すべての分岐で同じ時間を要する実装
- ダミー演算の挿入

**3. 並列化**：
- CRT復号化では、$m_p$ と $m_q$ の計算を並列実行可能
- 複数コアでの高速化

これらの最適化技術により、RSA暗号は実用的な性能を実現し、現代の暗号システムの基盤となっています。
...
逆に、RSA問題を解くアルゴリズムが存在すれば、それを利用して素因数分解を行うことも可能です。この相互還元可能性により、RSA暗号の安全性は素因数分解問題の困難性と等価であることが示されます。
...
**Carmichael関数** $\lambda(n)$ は、正整数 $n$ に対して以下のように定義されます。

$$\lambda(n) = \text{lcm}(\lambda(p_1^{e_1}), \lambda(p_2^{e_2}), \ldots, \lambda(p_k^{e_k}))$$

ここで、$n = p_1^{e_1} p_2^{e_2} \cdots p_k^{e_k}$ は $n$ の素因数分解です。
...
**高次剰余**：整数 $a$ が $n$ を法とする $k$ 次剰余であるとは、$x^k \equiv a \pmod{n}$ なる整数 $x$ が存在することです。

**性質**：$n = pq$（$p, q$ は素数）のとき、$a$ が $n$ を法とする $k$ 次剰余である必要十分条件は、$a$ が $p$ と $q$ を法とする $k$ 次剰余であることです。
...
**証明**：平文 $m_1, m_2 \in \mathbb{Z}_n$ と対応する暗号文 $c_1, c_2$ を考えます。
