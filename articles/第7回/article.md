...
* [7.4 多変数多項式暗号（Multivariate Cryptography）](#74-多変数多項式暗号-Multivariate-Cryptography)
* [7.5 同種写像暗号（Isogeny-Based Cryptography）](#75-同種写像暗号-Isogeny-Based-Cryptography)
* [7.6 符号ベース暗号（Code-Based Cryptography）](#76-符号ベース暗号-Code-Based-Cryptography)
* [7.7 ハッシュベース署名（Hash-Based Signatures）](#77-ハッシュベース署名-Hash-Based-Signatures)
...
1994年にPeter Shorが発表した**Shorのアルゴリズム** は、量子コンピュータ上で素因数分解と離散対数問題を多項式時間で解く画期的なアルゴリズムです。
...
AESなどの対称鍵暗号は、Groverのアルゴリズムによって鍵長が実質的に半分になります。例えば、AES-256は量子コンピュータ上では128ビットの安全性しか持ちません。
ただし、これは対称鍵暗号の鍵長を2倍にすることで対応できます（AES-256をAES-512にする）。
...
**特徴**：
- **高速性**：多項式演算により効率的な実装が可能
- **長年の研究実績**：1996年の提案以来、安全性評価が継続されている
- **標準化の状況**：NISTの標準化では選定されなかったものの、実用性が評価されている
...
**特徴**：
- 長年の研究により安全性が評価されている
- NISTの標準化では最終候補には選ばれませんでしたが、有力な候補の一つだった
- 公開鍵サイズが大きいことが課題
...
攻撃者は通信の傍受、データベース侵入、クラウドストレージからの窃取などにより暗号文を収集し、数十年にわたって保存する。量子コンピュータが実現した時点で、蓄積した暗号文を一括解読し、過去の機密情報を復元します。
