...
## 公開鍵暗号（TDP + ランダムオラクル G,H）に対する IND-CCA1 証明

ここでは、タグ \(c_3\) を追加した方式が IND-CCA1 を満たすことを示す。定義は `docs/004_asymmetric/security_definitions.md` を参照。

### 前提・モデル
- **攻撃モデル**: IND-CCA1（チャレンジ前のみ復号オラクルを許す）。
- **構成要素**:
...
### 定理
この方式は ROM（ランダムオラクルモデル）において IND-CCA1 安全である。すなわち任意の PPT 敵対者 \(\mathcal{A}\) に対し、ある反転器 \(\mathcal{B}\) が存在して
\[
 \mathrm{Adv}^{\mathsf{IND\text{-}CCA1}}_{\Pi,\,\mathcal{A}}(k)
...
まず、\(\overline{\textsf{AskG}}\) かつ \(\overline{\textsf{AskH}}\) のとき、敵の視点で \(G(r)\) は一様、かつ \(H(r,\cdot)\) は未照会であるため \(c_2\) は長さ \(k\) のワンタイムパッド、\(c_3\) はランダム値と区別不能である。チャレンジ後は復号オラクルが無い（CCA1）ので、\(c_3\) の整合性を検査する手段が無い。よって
\[
...
\]
ただし \(\epsilon_{\textsf{forge}}(k)\) は「敵が \(H\) を問合せずに有効タグを偽造してチャレンジ前の復号に通す」確率の上界で、\(\epsilon_{\textsf{forge}}(k)\le q_{\textsf{dec}}/2^k=\text{negl}(k)\)。
...
1) RO の実装: 遅延標本化でテーブル \(T_G,T_H\) を管理。
   ...
   - \(H(x,m)\): 未定義なら一様 \(V\leftarrow\{0,1\}^k\) で定義し返す。もし \(F(x)=y\) かつ（後述の）チャレンジで用いる \(m_b\) に等しければ、必要に応じて後で \(V\) を参照（プログラミングは不要。先に定義済みの値をそのまま使う）。

2) 復号オラクル（チャレンジ前）のシミュレーション: クエリ \((c_1,c_2,c_3)\) に対し、次を行う。
...
3) チャレンジ生成: \(\mathcal{A}_1\) から \((m_0,m_1)\) を受け取ったら、\(b\leftarrow\{0,1\}\)。
   ...
   - \(U\leftarrow\{0,1\}^k\) を一様に取り、\(c_2^*:=m_b\oplus U\)。このとき \(T_G\) のエントリ \(x\) で \(F(x)=y\) が未定義なら、後で \(G(r)\) を \(U\) にプログラムする（実際には \(r\) を同定した時点で \(T_G[r]:=U\) とすれば良い）。
...
- CCA1 ではチャレンジ後に復号オラクルが無いため、\(c_3\) の正当性を検査されない点が重要。CCA2 まで高めるには FO 変換（再ランダム化・整合性検査の工夫）等が必要。
